...
def test_http2_rapid_reset_vulnerable(mock_ssl, mock_create_conn):
    ...
    mock_ssock.recv.side_effect = TimeoutError("Connection dropped")
...
def test_graphql_nuclear_vulnerable(mock_session_class):
    ...
    baseline_resp = mock_requests_response(200, '{"data":{"__schema":{"types":[]}}}')
    mock_session = mock_session_class.return_value
    mock_session.post.side_effect = [
        baseline_resp,  # /graphql baseline
        requests.exceptions.Timeout("Timeout"),  # /graphql nuclear
    ]
...
def test_oidc_poisoning_vulnerable(mock_socket):
    mock_socket.return_value.recv.side_effect = [
        b'HTTP/1.1 201 Created\r\n\r\n{"client_id": "123", "logo_uri": "http://169.254.169.254"}',
        b"",
    ]
...
def test_coerced_auth_web_vulnerable(mock_socket):
    mock_socket.return_value.recv.side_effect = [
        b"HTTP/1.1 500 Internal Server Error\r\n\r\nThe network path was not found cascavel-test",
        b"",
    ]
