...
def generate_html(categories, lang="pt"):
    ...
    for cat in categories:
        ...
        html.append('<div class="plugin-category-block" style="margin-bottom: 2rem;">')
        html.append(
            f'<h3 style="color: var(--cyan); margin-bottom: 1rem; border-bottom: 1px solid rgba(0, 255, 229, 0.2); padding-bottom: 0.5rem;"><span class="cat-icon">{cat["icon"]}</span> {cat["name"]} <span class="badge-count" style="font-size: 0.8rem; vertical-align: middle; margin-left: 0.5rem;">{count}</span></h3>'
        )
        html.append('<div class="table-wrapper">')
        html.append('<table class="plugin-table">')

        if lang == "pt":
            html.append(
                '<thead><tr><th style="width: 25%;">Plugin</th><th style="width: 25%;">Arquivo</th><th>Técnicas</th></tr></thead>'
            )
        else:
            html.append(
                '<thead><tr><th style="width: 25%;">Plugin</th><th style="width: 25%;">File</th><th>Techniques</th></tr></thead>'
            )

        html.append("<tbody>")
        for p in cat["plugins"]:
            html.append(
                f'<tr><td><strong>{p["name"]}</strong></td><td><code>{p["file"]}</code></td><td class="techniques">{p["techniques"]}</td></tr>'
            )
        html.append("</tbody>")
        html.append("</table>")
        html.append("</div>")
