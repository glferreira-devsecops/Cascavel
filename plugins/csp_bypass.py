...
def _parse_csp(target):
    ...
    for page in PAGES_TO_CHECK:
        try:
            ...
            meta_csp = re.findall(r'<meta[^>]*content-security-policy[^>]*content="([^"]+)"',
...
def _analyze_directives(csp_string, page):
    ...
    if "base-uri" not in directives:
        vulns.append({
            "tipo": "CSP_NO_BASE_URI", "pagina": page,
            "severidade": "ALTO",
            "descricao": "base-uri ausente — dangling markup + base tag injection!",
        })
    if "frame-ancestors" not in directives:
        vulns.append({
            "tipo": "CSP_NO_FRAME_ANCESTORS", "pagina": page,
            "severidade": "MEDIO",
    ...
    if critical_missing:
        vulns.append({
            "tipo": "CSP_MISSING_DIRECTIVES", "pagina": page,
            "directives": critical_missing, "severidade": "ALTO",
        })
    ...
    for jsonp_domain in JSONP_BYPASS_DOMAINS:
        ...
        if domain in csp_string:
            vulns.append({
                "tipo": "CSP_JSONP_BYPASS", "pagina": page,
                "dominio": jsonp_domain, "severidade": "CRITICO",
    ...
    for cdn_domain, desc in FRAMEWORK_BYPASS_DOMAINS:
        if cdn_domain in csp_string:
            vulns.append({
                "tipo": "CSP_CDN_BYPASS", "pagina": page,
                "dominio": cdn_domain, "severidade": "ALTO",
                "descricao": desc,
            })
    ...
    if "'strict-dynamic'" in csp_string and "'unsafe-inline'" in csp_string:
        vulns.append({
            "tipo": "CSP_STRICT_DYNAMIC_INLINE", "pagina": page,
            "severidade": "MEDIO",
    ...
    if nonces:
        for nonce in nonces:
            if len(nonce) < 8:
                vulns.append({
                    "tipo": "CSP_SHORT_NONCE", "pagina": page,
                    "nonce_len": len(nonce), "severidade": "ALTO",
    ...
    if "report-uri" in csp_string or "report-to" in csp_string:
        ...
        if report_match:
            vulns.append({
                "tipo": "CSP_REPORT_URI_EXPOSED", "pagina": page,
                "uri": report_match.group(1)[:80], "severidade": "BAIXO",
...
def _check_report_only(csp_ro, page):
    ...
    if csp_ro:
        return [{
            "tipo": "CSP_REPORT_ONLY", "pagina": page,
            "severidade": "ALTO",
...
def run(target, ip, open_ports, banners):
    ...
    return {
        "plugin": "csp_bypass",
        "versao": "2026.1",
        "tecnicas": ["jsonp_bypass", "cdn_bypass", "base_uri_missing",
                      "frame_ancestors", "strict_dynamic", "nonce_analysis",
                      "report_only", "meta_conflict", "directive_analysis"],
