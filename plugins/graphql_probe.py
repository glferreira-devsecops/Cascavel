...
def _test_debug(url, ep, resultado):
    try:
        resp = requests.post(
            url,
            json={"query": "{ __typename }", "extensions": {"tracing": True}},
            timeout=5,
...
def _test_field_suggestion(url, ep, resultado):
    try:
        ...
        if resp.status_code in [200, 400] and "Did you mean" in resp.text:
            resultado["vulns"].append(
                {
                    "tipo": "FIELD_SUGGESTION",
                    "endpoint": ep,
                    "severidade": "BAIXO",
                    "descricao": "Field suggestions expostas — schema enumeration!",
