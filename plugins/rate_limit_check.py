...
def run(target, ip, open_ports, banners):
    ...
    endpoints = ["/login", "/api/login", "/api/auth", "/register",
    ...
    for ep in endpoints:
        ...
        for i in range(30):
            try:
                start = time.time()
                resp = requests.post(url, json={"username": f"test{i}", "password": "test"},
                                     timeout=5)
        ...
        if not blocked and len(statuses) >= 25:
            resultado[ep] = {
                ...
                "tempo_medio": round(sum(times) / len(times), 3) if times else 0,
            }

    return {"plugin": "rate_limit_check", "resultados": resultado if resultado else "Nenhum endpoint de auth encontrado"}
