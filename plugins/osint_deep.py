...
# Documentation paths
DOC_PATHS = [
    "/docs", "/documentation", "/api-docs", "/wiki",
    "/swagger.json", "/openapi.json", "/openapi.yaml",
    "/redoc", "/swagger-ui/", "/graphql",
    "/postman/collection.json", "/insomnia.json",
    "/CHANGELOG.md", "/README.md", "/CONTRIBUTING.md",
    "/SECURITY.md", "/LICENSE",
    "/admin", "/admin/docs", "/internal",
...
SQUAT_PATTERNS = [
    "{base}secure.com", "{base}login.com", "{base}auth.com",
    "{base}-app.com", "{base}-api.com", "{base}-dev.com",
    "{base}support.com", "{base}help.com", "{base}portal.com",
    "{base}mail.com", "{base}-mail.com", "{base}web.com",
    "{base}online.com", "{base}net.com", "{base}org.com",
    "my{base}.com", "get{base}.com", "{base}hq.com",
    "{base}-inc.com", "{base}.co", "{base}.io",
]
...
def _check_social_profiles(target):
            ...
            for line in resp.text.splitlines():
                if line.lower().startswith("disallow:"):
                    ...
                    if path and any(kw in path.lower() for kw in [
                        "admin", "private", "secret", "internal", "api",
                        "config", "backup", "debug", "test", "staging",
                    ]):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            social = []
            for platform in ["github.com", "gitlab.com", "bitbucket.org", "twitter.com",
...
def _check_leaked_credentials(target):
    ...
    env_paths = [
        ...
        "/.env.backup", "/.env.old", "/env", "/environment",
    ...
    for path in env_paths:
        try:
            ...
            if resp.status_code == 200:
                body = resp.text
                if any(kw in body.upper() for kw in [
                    "KEY", "SECRET", "TOKEN", "PASSWORD", "DATABASE_URL",
                    "AWS_", "API_KEY", "PRIVATE_KEY", "SMTP_",
                ]):
                    findings.append({
                        "tipo": "LEAKED_ENV",
                        "path": path,
