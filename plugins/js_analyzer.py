...
def _scan_secrets(content, filename):
    ...
    for secret_name, pattern in SECRET_PATTERNS.items():
        ...
        for match in matches[:3]:
            ...
            findings.append({
                "tipo": secret_name, "arquivo": filename,
                "valor_parcial": str(match)[:15] + "...", "severidade": sev,
            })
    return findings
...
def _check_supply_chain(content, js_url, filename):
    ...
    if "//# sourceMappingURL=" in content:
        ...
        if map_url:
            vulns.append({
                "tipo": "SOURCE_MAP_EXPOSED", "arquivo": filename,
    ...
    if "integrity=" not in content and "cdn" in js_url.lower():
        vulns.append({
            "tipo": "SEM_SRI", "arquivo": filename, "severidade": "MEDIO",
