...
INJECTION_HEADERS = [
    ...
    "Authorization", "X-Client-IP", "X-Remote-IP", "X-Remote-Addr",
    "X-Originating-IP", "Contact", "Forwarded", "Origin",
]
    ...
    for header in INJECTION_HEADERS:
        ...
        for payload_tpl in JNDI_PAYLOADS[:2]:  # Limita a 2 payloads por header
            ...
            try:
                ...
                results.append({
                    "header": header, "payload": payload[:50] + "...",
                ...
                results.append({
                    ...
                    "status": "TIMEOUT", "nota": "Timeout pode indicar processamento JNDI",
                })
            except Exception:
                continue
    return results
...
def _detect_java_indicators(url, timeout):
    ...
    try:
        ...
        if any(kw in r.text.lower() for kw in [
            "java.lang", "javax.", "org.apache", "org.springframework",
            "stacktrace", "tomcat", ".jsp",
        ]):
...
def run(target, ip, open_ports, banners):
    ...
    # 3. Path injection
    path_result = _test_path_injection(url, callback_base, timeout)
    ...
    if suspicious_timeouts:
        vulns.append({
            "tipo": "LOG4J_TIMEOUT_ANOMALY", "severidade": "ALTO",
            "count": len(suspicious_timeouts),
            "descricao": f"{len(suspicious_timeouts)} headers causaram timeout — possível processamento JNDI! Configure OOB callback para confirmar.",
        })

    if slow_responses:
        vulns.append({
            "tipo": "LOG4J_SLOW_RESPONSE", "severidade": "MEDIO",
    ...
    if java_indicators:
        vulns.append({
            "tipo": "JAVA_STACK_DETECTED", "severidade": "INFO",
            "indicators": java_indicators,
            "descricao": "Java/Spring/Tomcat detectado — alvo potencial para Log4Shell",
        })

    return {
        "plugin": "cve_2021_44228_scanner", "versao": "2026.1",
        "tecnicas": ["jndi_header_injection", "waf_bypass_payloads",
                      "path_injection", "java_fingerprint", "oob_token_generation",
