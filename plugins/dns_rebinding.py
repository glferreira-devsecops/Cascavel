...
def run(target, ip, open_ports, banners):
    ...
    return {
        ...
        "versao": "2026.1",
        "tecnicas": ["ttl_analysis", "wildcard_dns", "multiple_a",
                      "dnssec", "email_security", "zone_transfer",
                      "caa_records", "ns_analysis", "zero_ip_rebinding"],
...
def _check_multiple_a(target, resultado):
    ...
    try:
        ...
        if len(unique_ips) > 1:
            ...
            internal = [ip for ip in unique_ips
...
def _check_dnssec(target, resultado):
    ...
    if "RRSIG" not in output:
        resultado["vulns"].append({
            "tipo": "DNSSEC_AUSENTE", "severidade": "MEDIO",
...
def _check_email_security(target, resultado):
    ...
    for domain, rtype, label, expected in dns_sec_records:
        ...
        if not output or expected not in output:
            ...
            resultado["vulns"].append({
                "tipo": f"{label}_AUSENTE", "severidade": sev,
            ...
            if label == "SPF" and "+all" in output:
                resultado["vulns"].append({
                    "tipo": "SPF_ALLOW_ALL", "severidade": "CRITICO",
...
def _check_caa(target, resultado):
    ...
    if not output:
        resultado["vulns"].append({
            "tipo": "CAA_AUSENTE", "severidade": "BAIXO",
...
def _check_ns_records(target, resultado):
    ...
    if output:
        ...
        if len(ns_servers) < 2:
            resultado["vulns"].append({
                "tipo": "DNS_SINGLE_NS", "severidade": "MEDIO",
...
def _check_zero_ip_rebinding(target, resultado):
    ...
    try:
        ...
        if ip == "0.0.0.0":
            resultado["vulns"].append({
                "tipo": "DNS_ZERO_IP", "severidade": "CRITICO",
                "descricao": "Domínio resolve para 0.0.0.0 — DNS rebinding bypass!",
            })
        elif ip.startswith("127."):
            resultado["vulns"].append({
                "tipo": "DNS_LOCALHOST_RESOLVE", "ip": ip,
                "severidade": "ALTO",
