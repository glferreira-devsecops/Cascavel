...
def _analyze_token(token, resultado):
    ...
    if alg in ("RS256", "RS384", "RS512"):
        resultado["vulns"].append(
            {
                "tipo": "JWT_ALG_CONFUSION_SURFACE",
                "severidade": "MEDIO",
                "alg": alg,
    ...
    if "kid" in header:
        ...
        if "/" in kid or ".." in kid:
            resultado["vulns"].append(
                {
                    "tipo": "JWT_KID_PATH_TRAVERSAL",
                    "severidade": "CRITICO",
                    "kid": kid,
                    "descricao": "kid contém path traversal!",
                }
            )
        # kid SQLi check
            ...
            resultado["vulns"].append(
                {
                    ...
                    "severidade": "CRITICO",
                    "kid": kid,
                    "descricao": "kid contém caracteres SQL — possível SQLi!",
                }
            )
        resultado["vulns"].append(
            {
                "tipo": "JWT_KID_PRESENTE",
                "severidade": "BAIXO",
    ...
    for header_key in ["jwk", "jku", "x5u", "x5c"]:
        if header_key in header:
            resultado["vulns"].append(
                {
                    "tipo": f"JWT_{header_key.upper()}_INJECTION",
                    "severidade": "CRITICO",
            ...
            resultado["vulns"].append(
                {
                    ...
                    "severidade": "ALTO",
                    "exp": exp,
                    "descricao": "Token expirado — testar se servidor ainda aceita",
                }
            )
        elif exp - now > 86400 * 365:
            resultado["vulns"].append(
                {
                    "tipo": "JWT_LONG_EXPIRY",
                    "severidade": "MEDIO",
    ...
    if "iat" in payload and payload["iat"] > now + 86400:
        resultado["vulns"].append(
            {
                "tipo": "JWT_BACK_TO_FUTURE",
                "severidade": "ALTO",
                "iat": payload["iat"],
                "descricao": "iat no futuro — 'Back to the Future' attack!",
            }
        )
    ...
    for claim in ["role", "roles", "is_admin", "isAdmin", "admin", "permissions", "scope"]:
        if claim in payload:
            resultado["vulns"].append(
                {
                    "tipo": "JWT_PRIVILEGE_CLAIM",
                    "severidade": "MEDIO",
                    "claim": claim,
                    "valor": str(payload[claim])[:50],
