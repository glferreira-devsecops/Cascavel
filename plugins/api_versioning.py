...
API_PREFIXES = [
    "/api", "/v1", "/v2", "/v3", "/v4", "/v0",
    "/api/v1", "/api/v2", "/api/v3", "/api/v4", "/api/v0",
    "/api/beta", "/api/alpha", "/api/internal", "/api/legacy",
    "/api/dev", "/api/staging", "/api/test", "/api/private",
    "/api/admin", "/api/debug",
]

DEPRECATED_INDICATORS = ["deprecated", "sunset", "end-of-life", "legacy",
                          "x-api-warn", "x-api-deprecated", "obsolete"]

HEADER_VERSION_HEADERS = [
    "X-API-Version", "API-Version", "X-Version",
    "Accept-Version", "X-Api-Revision",
]
...
def _enumerate_versions(target):
            ...
            if resp.status_code in (200, 301, 302, 401, 403):
                ...
                for indicator in DEPRECATED_INDICATORS:
                    if indicator in headers_lower or indicator in body_lower:
                        vulns.append({
                            "tipo": "API_DEPRECATED_ACTIVE", "endpoint": prefix,
                            "severidade": "ALTO", "indicador": indicator,
                ...
                if sunset:
                    vulns.append({
                        "tipo": "API_SUNSET_HEADER", "endpoint": prefix,
                        "sunset": sunset, "severidade": "MEDIO",
                        "descricao": f"API tem Sunset header: {sunset}",
                    })

                # Check for different auth requirements
                if resp.status_code == 200 and ("internal" in prefix or "admin" in prefix
                                                 or "debug" in prefix or "private" in prefix):
                    vulns.append({
                        "tipo": "SHADOW_API_EXPOSED", "endpoint": prefix,
                        "severidade": "CRITICO",
    ...
    if len(active_versions) > 3:
        vulns.append({
            "tipo": "API_MULTIPLE_VERSIONS", "versoes": len(active_versions),
            "severidade": "MEDIO",
...
def _check_old_version_vulns(target, active_versions):
    ...
    if responses:
        ...
        if max(sizes) > min(sizes) * 2:
            vulns.append({
                "tipo": "VERSION_DATA_LEAK", "severidade": "ALTO",
...
def run(target, ip, open_ports, banners):
    ...
    return {
        ...
        "versao": "2026.1",
        "tecnicas": ["version_enum", "deprecated_detection", "sunset_header",
                      "shadow_api", "header_versioning", "version_data_leak"],
