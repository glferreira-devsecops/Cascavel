...
PAGES = [
    "/", "/login", "/register", "/contact", "/settings", "/profile",
    "/admin", "/account", "/api/", "/password/reset", "/payment",
    "/checkout", "/transfer", "/delete-account", "/api/settings",
]

CSRF_TOKEN_NAMES = [
    "csrf", "csrftoken", "csrf_token", "_csrf", "xsrf",
    "xsrf-token", "authenticity_token", "__requestverificationtoken",
    "csrfmiddlewaretoken", "antiforgerytoken", "_token",
    "csrf-token", "x-csrf-token",
]

SENSITIVE_ACTIONS = [
    "password", "delete", "transfer", "payment", "admin",
    "settings", "update", "email", "phone", "role",
]
            ...
            if token_match:
                ...
                if entropy < 3.5:
                    vulns.append({
                        ...
                        "pagina": page, "entropia": round(entropy, 2),
                        "severidade": "ALTO",
                ...
                if len(token_val) < 16:
                    vulns.append({
                        ...
                        "pagina": page, "tamanho": len(token_val),
                        "severidade": "ALTO",
...
def _analyze_cookies(resp, page):
    ...
    for cookie_part in set_cookies.split(","):
        ...
        if "httponly" not in lower:
            vulns.append({
                "tipo": "COOKIE_SEM_HTTPONLY", "pagina": page,
                "severidade": "ALTO",
        ...
        if "secure" not in lower:
            vulns.append({
                "tipo": "COOKIE_SEM_SECURE", "pagina": page,
                "severidade": "MEDIO",
...
def _check_json_csrf(target, page):
    ...
    try:
        ...
        if resp.status_code in (200, 201, 204):
            vulns.append({
                "tipo": "JSON_CSRF_TEXT_PLAIN", "pagina": page,
                "severidade": "ALTO",
    ...
    try:
        ...
        if resp.status_code in (200, 201, 204):
            vulns.append({
                "tipo": "JSON_CSRF_FORM_URLENCODED", "pagina": page,
                "severidade": "ALTO",
...
def _check_cors_csrf(target):
    ...
    try:
        ...
        if acao == "https://evil.com" and acac.lower() == "true":
            return {
                "tipo": "CORS_CSRF_ENABLED", "severidade": "CRITICO",
...
def _check_referer_validation(target):
    ...
    for page in ["/settings", "/profile", "/api/settings"]:
        try:
            # Request sem Referer
            resp_no = requests.post(f"http://{target}{page}",
                                     data={"test": "csrf"}, timeout=5,
                                     headers={"Referer": ""})
            # Request com Referer evil
            resp_evil = requests.post(f"http://{target}{page}",
                                       data={"test": "csrf"}, timeout=5,
                                       headers={"Referer": "https://evil.com/"})
            if resp_no.status_code in (200, 201, 204) or resp_evil.status_code in (200, 201, 204):
                vulns.append({
                    "tipo": "CSRF_NO_REFERER_CHECK", "pagina": page,
                    "severidade": "MEDIO",
...
def run(target, ip, open_ports, banners):
    ...
    return {
        ...
        "versao": "2026.1",
        "tecnicas": ["form_analysis", "token_entropy", "samesite_detection",
                      "json_csrf", "sendbeacon", "cors_csrf", "referer_bypass",
                      "cookie_flags"],
