...
def _check_alt_svc_header(target: str, session: requests.Session, ports: list[int]) -> list[dict[str, Any]]:
                    ...
                    findings.append(
                        {
                            ...
                            "descricao": (
                                f"Alt-Svc header: {alt_svc}. HTTP/3 available on port {alt_port}, max-age: {max_age}."
                            ),
                            "header": "Alt-Svc",
                            "value": alt_svc,
                            "alt_port": alt_port,
                ...
                findings.append(
                    {
                        ...
                        "remediacao": ('Add Alt-Svc header to advertise HTTP/3: Alt-Svc: h3=":443"; ma=86400'),
                    }
                )
...
def _check_h3_downgrade(target: str, session: requests.Session, ports: list[int]) -> list[dict[str, Any]]:
    """Test for HTTP/3 downgrade attack vectors."""
    findings: list[dict[str, Any]] = []

    check_url = (
        f"https://{target}" if 443 in ports else f"https://{target}:{ports[0]}" if ports else f"https://{target}"
    )
...
def _check_quic_config(target: str, ip: str, ports: list[int]) -> list[dict[str, Any]]:
...
def _check_h3_vulnerabilities(target: str, session: requests.Session, ports: list[int]) -> list[dict[str, Any]]:
...
def _check_h2_h3_migration(target: str, session: requests.Session, ports: list[int]) -> list[dict[str, Any]]:
